...
$btn.bsButton('toggle')})
$(document).ready(function(){$(".rowCount").each(function(){var count=$(this).text().replace("(","").replace(")","");$(this).text(count);$(this).addClass("pull-right badge");});loadInbox();$("body").swipe({swipeRight:function(event,direction,distance,duration,fingerCount,fingerData){var posx=fingerData[0]['start']['x'];if($(".hi-trigger").is(":visible")&&!$("body").hasClass("sidebar-toggled")&&posx<20){$(".hi-trigger").trigger("click");}},swipeLeft:function(event,direction,distance,duration,fingerCount){if($(".ma-backdrop").is(":visible")&&$("body").hasClass("sidebar-toggled")){$(".ma-backdrop").trigger("click");}},preventDefaultEvents:false,fallbackToMouseEvents:false,});var toogleMenu=function(menu){if($(menu).parent().css("display")!=="inline-block"){$(menu).next().slideToggle(200);$(menu).parent().toggleClass("toggled");}};var initMenu=function(){$("#sidebar a.dropdown").each(function(){$(this).parent().addClass("toggled");if(!$(this).parent().hasClass("active")){toogleMenu(this);}});$("#sidebar a.dropdown").on("click",function(e){toogleMenu(this);e.preventDefault();e.stopPropagation();});};initMenu();var originalHash='';$("#sidebar-trigger").on("click",function(){originalHash=location.hash.replace('#','');$(window).bind('hashchange.menu',function(event){var hash=location.hash.replace('#','');if(hash!=='menu'&&$(".ma-backdrop").is(":visible")&&$("body").hasClass("sidebar-toggled")){$(".ma-backdrop").trigger("click");}});$("body").on("click.sidebar-toggled",function(e){var container=$("#sidebar");if($(".ma-backdrop").is(":visible")&&$("body").hasClass("sidebar-toggled")&&!$("#sidebar-trigger").is(e.target)&&$("#sidebar-trigger").has(e.target).length===0&&!container.is(e.target)&&container.has(e.target).length===0){$("body").removeClass("sidebar-toggled");$(".ma-backdrop").remove();$("#sidebar, #sidebar-trigger").removeClass("toggled");$("body").off(".sidebar-toggled");$(window).unbind('hashchange.menu');location.hash=originalHash;return false;}});var backdrop='<div class="ma-backdrop" />';$("body").addClass("sidebar-toggled");$("header.navbar").append(backdrop);$(this).addClass("toggled");$("#sidebar").addClass("toggled");location.hash='menu';});if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){$("html").addClass("ismobile");}
$(window).load(function(){Waves.init();attachButtonEffect();setTimeout(function(){$(".page-loader").fadeOut();},500);});var scrollBar=function(selector,theme,mousewheelaxis){$(selector).mCustomScrollbar({theme:theme,scrollInertia:100,axis:"mousewheelaxis",mouseWheel:{enable:!0,axis:mousewheelaxis,preventDefault:!0}});};if($("#sidebar").length>0){scrollBar("#sidebar","minimal-dark","y");}
